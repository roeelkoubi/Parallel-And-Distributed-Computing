...
// Read the First Value from the File (Matching Value)
    ...
    void arrayResultToFile(int* resultArray, FILE* filename, int pictureId, int ObjectCounter) 
    {
        if (ObjectCounter == 9)
        {
...
int main(int argc, char* argv[]) {
	...
	if (my_rank == 0)
	{
            ...
            do {
                ...
                arrayResultToFile(resultArray,filePointerOutput,pictureId,objectCounter);
                numberOfProccessCurrentlyWorking--;
